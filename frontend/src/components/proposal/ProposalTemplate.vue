<template>
  <div>
    <div class="row gx-5">
      <div
        class="col-md-12 col-lg-4 order-lg-last lh-sm pb-5"
        id="">
        <StickySidebar>
          <div
            class=" "
            style="">
            <div
              class="card bg-body-tertiary p-2 d-flex gap-2"
              ...
              :class="[
                ...
                <button
                  v-for="(item, idx) in contentTabs"
                  :key="item.formName"
                  @click="selectTab(idx)"
                  class="list-group-item list-group-item-action d-flex justify-content-between align-items-center lh-sm"
                  :class="
                    [
                      isActive(idx) ? ' fw-bold border border-dark-subtle' : '',
                      item.hasError || (isNaN(item.progress) && item.useTaskForm)
                        ? 'list-group-item-danger border-danger border-1'
                        : !isNaN(item.progress) && item.progress >= this.neededPercentages
                          ? 'list-group-item-success'
                  ...
                  <div>
                    ...
                    <span v-else
                      ><span v-if="!isNaN(item.progress)">{{ item.progress }}%</span>
                    </span>
                  </div>
                </button>
              </ul>
              ...
              <div
                ...
                :class="[
                  this.returnFormProgress >= this.neededPercentages
                    ? 'bg-success-subtle'
                    : 'bg-warning-subtle',
                ]">
                <div class="card-body">
                  ...
                  <div
                    v-if="this.returnFormProgress >= this.neededPercentages"
                    class="d-flex justify-content-between">
                    <div>
                      <p>Ready to be generated</p>
                      <p class="muted small">
                        At least {{ neededPercentages }}% of the required<span class="text-danger"
                          >*</span
                        >
                        fields filled in each category.
                      </p>
                      <p class="muted small">The proposal is ready to be generated.</p>
                    </div>
                    <div>✔️</div>
                  </div>
                  <div
                    ...
                    <div>
                      ...
                      <p class="muted small">
                        To submit a proposal, you must create at least one task and complete
                        {{ neededPercentages }}% of the required<span class="text-danger">*</span>
                        fields in each category.
                      </p>
                    </div>
          ...
          -->
              ...
              <div class="py-0">
                <AutoSaveInterval
                  :timeout="30000"
                  :show="canSaveProposal"
                  @end-of-time-period="saveProposal" />
              </div>

              <button
                :disabled="!canSaveProposal"
                class="btn btn-success"
                @click="saveProposal((showMessage = true))">
      ...
      <div
        class="col-md-12 col-lg-8 mb-5"
        id="content">
        <div
          class="alert alert-secondary bg-body-tertiary text-center py-2 mb-1 fw-semibold"
          role="alert">
          <i class="bi bi-info-circle"></i> We recommend keeping a local copy of the content you
          create here.
        </div>
        <div
          class="alert alert-warning bg-warning-subtle text-center py-2 mb-3 fw-semibold"
          role="alert">
          <i class="bi bi-exclamation-triangle"></i> Please don't open this page from multiple
          borser windows.
        </div>
        <div
          ...
          <div v-show="isActive(index)">
            ...
            <div v-if="tab.useTaskForm">
              <TaskForm
                :name="tab.formName"
                :show-action-btn="false"></TaskForm>
              <div
                class="card p-3 bg-light-subtle"
                :hidden="!taskAvailable">
                <h3><i class="bi bi-file-earmark-text"></i> Fill task details: <br /><br /></h3>

                <VueForm
                  v-if="taskAvailable"
                  :show-missing-input-top="true"
                  :name="tab.formName"
                  :ref="tab.formName"
                  ...
                  <VueTextSection
                    ...
                    <template #title
                      >{{ question.title
                        ...
                        v-if="question.validation.required"
                        class="text-danger"
                        ><b>*</b></small
                      ></template
                    >
                    ...
                    <VueInput
                      ...
                      v-for="(task, index) in question.value"
                      :key="`taskId${task.task_name + index}`"
                      :questionKey="question.key"
                      :uniqueId="question.value[index].uniqueId"
                      :label="`<span class='badge ${
                        badgeClasses[index % badgeClasses.length]
                      } me-1'>Task ${index + 1}</span>`"
                      :has-error="question.value[index].errorState"
                      :type="question.type"
                      :options="question.options"
                      :text-min-length="question.validation.minlength"
                      :text-max-length="question.validation.maxlength"
                      :required="question.validation.required"
                      :allow-special-characters="question.validation.allowSpecialCharacters"
                      :pattern="question.validation.pattern"
                      :icon="question.icon"
                      v-model="question.value[index].value"
        ...
        <div class="d-flex gap-5 my-5 justify-content-between align-content-center">
          <button
            ...
            @click="activeTab--"
            type="button"
            class="btn btn-lg w-25"
            :class="activeTab === 0 ? 'btn-outline-light' : 'btn-outline-primary'">
            <i class="bi bi-arrow-left"></i>Back
          </button>
          <button
            :disabled="activeTab === contentTabs.length - 1 || (activeTab === 2 && !taskAvailable)"
            @click="activeTab++"
            type="button"
            class="btn btn-lg w-25"
            :class="
              activeTab === contentTabs.length - 1 ? 'btn-outline-light' : 'btn-outline-primary'
            ">
    ...
    <div
      class="modal fade"
      id="submitProposalPopup"
      data-bs-backdrop="static"
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="submitProposalPopupLabel"
      aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            <p>
              Once generated, you will be able to download the PDF file. It will also be sent by
              e-mail.
            </p>
...
export default {
  ...
  computed: {
    ...
    canSubmitProposal() {
      ...
      //   return false;
      // }
      if (tasksLength === 0 || !this.userReachedNeededPercentages || this.proposalContainsError) {
        return false
      }
      ...
      this.contentTabs.flatMap((tab) => {
        if (!tab.useTaskForm) {
          amountOfForms =
            amountOfForms +
            tab.questions
              .flatMap((y) => y.validation && y.validation.required)
              .reduce(function (a, b) {
                return a + b
              }, 0)
        } else {
          amountOfForms =
            ...
            tab.questions
              .flatMap((question) =>
                question.value.flatMap((y) => y.validation && y.validation.required)
              )
              .reduce(function (a, b) {
                return a + b
              }, 0)
      ...
      this.contentTabs.flatMap((tab) => {
        if (!tab.useTaskForm) {
          ...
          sum =
            sum +
            tab.questions
              .flatMap((question) => question.value.flatMap((y) => y.mapIsAnswered))
              .reduce(function (a, b) {
                return a + b
              }, 0)
        }
  ...
  methods: {
          ...
          proposalItem.questions.forEach((x) => {
            ...
            if (x.type === 'text' && x.convertToList && !Array.isArray(x.value)) {
              ...
              for (let i = 0; i < x.value.length; i++) {
                if (
                  x.value[i].type === 'text' &&
                  x.value[i].convertToList &&
                  !Array.isArray(x.value[i].value)
                ) {
          ...
          if (proposalItem.formName === 'Tasks') {
            ...
            await apiPut(`/challenge/${proposalStore.getProposalId}/update`, container, {
              ...
              .then(() => {
                if (showMessage) {
                  useToastAlertStore().showAlert(
                    'Challenge proposal updated successfully.',
                    'success'
                  )
                }
              })
    ...
    returnProgress(form) {
      ...
      if (form.useTaskForm) {
        const requiredQuestions = form.questions.flatMap((question) =>
          (question.value || []).filter(
            (subQuestion) => subQuestion.validation && subQuestion.validation.required
          )
        )
        ...
        const mapIsAnswered = form.questions.map((x) => {
          if (tasksLength === 0) {
            ...
            // x.value.forEach((y) => y.mapIsAnswered = 0);
            x.value.map((y) =>
              y.value === '' || y.value === null ? (y.mapIsAnswered = 0) : (y.mapIsAnswered = 1)
            )
        ...
        const requiredQuestions = form.questions.filter(
          (question) => question.validation && question.validation.required
        )
        const questionLength = requiredQuestions.length || 0
        if (questionLength > 0) {
          const mapIsAnswered = form.questions.map((x) => {
...
<style>
.badge-color-1 {
  background-color: #480010ff !important;
  color: white;
}
.badge-color-2 {
  background-color: #df0054 !important;
  color: white;
}
.badge-color-3 {
  background-color: #fc4e4eff !important;
  color: white;
}
/* .badge-color-4 {
