...
def configure_logging() -> None:  # pragma: no cover
    ...
    logger.add(sys.stderr, level="INFO", colorize=False, format=fmt)
    logger.add(
        f"{settings.log_folder}/api_logs.log",
        level=settings.log_level.value,
        retention="3 months",
        format=fmt,
        rotation="00:00",
        filter=filter_sqlite,
    )


def filter_sqlite(record):
    # Exclude records that mention 'sqlite3'
    return "sqlite3" not in record["message"]
