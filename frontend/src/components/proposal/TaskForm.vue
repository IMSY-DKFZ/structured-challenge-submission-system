...
<div class="alert alert-danger" v-if="errorMessage" role="alert">
  {{ errorMessage }}
    ...
    <div
      ...
      id="accordionCreateTask">
      <div class="accordion-item bg-light-subtle border-success-subtle">
        <h2 class="accordion-header">
          <button
            class="accordion-button bg-success-subtle"
            :class="{ collapsed: taskAvailable }"
            type="button"
            data-bs-toggle="collapse"
            data-bs-target="#collapseAccordionCreateTask"
            aria-expanded="false"
        ...
        <div
          id="collapseAccordionCreateTask"
          class="accordion-collapse collapse"
          :class="{ show: !taskAvailable }"
          data-bs-parent="#accordionCreateTask">
          <div class="accordion-body">
            <VueForm
              :action-btn-fullwidth="false"
              action-btn="Create a task"
              @submit="createTask"
              name="createTask"
              class="">
              <VueTextSection size="h6">
                <template #text>
                  <p>
                    <b>Task</b>: A challenge may deal with multiple different tasks for which
                    separate assessment results are provided. For example, a challenge may target
                    the problem of segmentation of human organs in computed tomography (CT) images.
                    It may include several tasks corresponding to the different organs of interest.
                  </p>
                  <div class="alert alert-primary">
                    <i class="bi bi-info-circle"></i> <b>Tip</b>: Our recommendation is to create
                    all tasks first and then fill in the necessary information. This way you can
                    complete everything in one step and save time.
                  </div>
                </template>
                <VueInput
                  v-model="newTaskName"
    ...
    <div
      class="accordion mb-3"
      id="accordionDeleteTask"
      :hidden="!taskAvailable">
      <div class="accordion-item bg-danger-subtle border-danger-subtle">
        <h4 class="accordion-header">
          <button
            class="accordion-button collapsed bg-danger-subtle text-danger"
            type="button"
            data-bs-toggle="collapse"
            data-bs-target="#collapseAccordionDeleteTask"
            aria-expanded="false"
        ...
        <div
          id="collapseAccordionDeleteTask"
          class="accordion-collapse collapse"
          data-bs-parent="#accordionDeleteTask">
          <div class="accordion-body">
            <p>
              You can delete your tasks here. Please note that, this is an
              <b>irreversible</b> action! All of the data will be deleted <b>permanently</b>.
            </p>

            <div
              class="px-3 py-1 my-2 d-flex justify-content-between align-items-center"
              v-for="(task, index) in proposalStore.tasks"
              :key="task.task_name">
              <div>
                <span class="badge bg-danger fs-6 p-2 pt-1">Task {{ index + 1 }} :</span>
                {{ task.task_name }}
              </div>
              <button
                class="btn btn-danger me-2"
                @click="deleteTask(task.id, task.task_name)">
                <i class="bi bi-trash2-fill"></i>
              </button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    async createTask() {
      if (tasks.value.map((x) => x.task_name).includes(this.newTaskName)) {
        ...
        if (typeof task_name === 'string' && task_name.length > 0) {
          await apiPost(
            `/task/create?challenge_id=${task_challenge_id}`,
            { task_name: task_name },
            {
    ...
    async deleteTask(id, name) {
      ...
      if (ok) {
        await apiDelete(`task/${id}/delete`)
          .then(() => {
